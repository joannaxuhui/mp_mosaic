...
class kd_tree
{
  ...
  private:
     ...
     */
	void quickselect(std::vector<point<Dim>>& newpoints, int first,int second,int currDim);
	void buildtree(std:: vector<point<Dim>>& ps,int begin, int end, int currDim);
	int  r(const point<Dim>& point1, const point<Dim>& point2) const;
	point<Dim>  find(const std::vector<point<Dim>>& ps, const point<Dim>& query ,int start, int end, int currDim, int& radius) const;
};
